...
namespace AnthonySantosInventoryManagementSystem
{
    public partial class AddPartScreen : Form
    {
        ...
        private async void ButtonPartSave_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(TextBoxPartID.Text) || !Int32.TryParse(TextBoxPartID.Text, out _))
            {
                MessageBox.Show("Please enter a valid numeric Part ID.", "Validation Error");
                return;
            }
            ...
            if (rbselected == 0)
            {
                MessageBox.Show("Please Select a Part's Source.", "Validation Error");
                return;
            }
            if (rbselected == 1 && (string.IsNullOrWhiteSpace(TextBoxX.Text) || !Int32.TryParse(TextBoxX.Text, out _)))
            {
                MessageBox.Show("Please enter a valid numeric Machine ID.", "Validation Error");
                return;
            }
            if (rbselected == 2 && string.IsNullOrWhiteSpace(TextBoxX.Text))
            {
            ...
            try
            {
                if (rbselected == 1)
                {
                    Inhouse x = new Inhouse(Int32.Parse(TextBoxPartID.Text), TextBoxPartName.Text, Double.Parse(TextBoxPartPriceCost.Text),
                        Int32.Parse(TextBoxPartInv.Text), Int32.Parse(TextBoxPartMin.Text), Int32.Parse(TextBoxPartMax.Text),
                        Int32.Parse(TextBoxX.Text));
                    Inventory.MyPartList.Add(x);
                }
                else
                {
                    Outsourced x = new Outsourced(Int32.Parse(TextBoxPartID.Text), TextBoxPartName.Text, Double.Parse(TextBoxPartPriceCost.Text),
                        Int32.Parse(TextBoxPartInv.Text), Int32.Parse(TextBoxPartMin.Text), Int32.Parse(TextBoxPartMax.Text),
                        TextBoxX.Text);
                    Inventory.MyPartList.Add(x);
                }

                // Call RepairOS API to save to database
                bool success = await ApiService.AddPartAsync(
                    TextBoxPartName.Text,
                    Double.Parse(TextBoxPartPriceCost.Text),
                    TextBoxSKU.Text,
                ...
                if (success)
                {
                    MessageBox.Show("Part added successfully to RepairOS!", "Success");
                }
                else
                {
                    MessageBox.Show("Local add done but API add failed.", "Warning");
                }
